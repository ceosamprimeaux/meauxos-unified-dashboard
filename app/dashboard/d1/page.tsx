...
import { executeD1Query, listD1Tables, getD1TableSchema, type D1QueryResult } from '@/lib/d1';

const API_BASE = process.env.NEXT_PUBLIC_API_URL ||
  'https://meauxcloudservices-365932368784.europe-west1.run.app';

export default function D1DatabasePage() {
  const [query, setQuery] = useState('SELECT * FROM sqlite_master WHERE type="table" LIMIT 10;');
  const [results, setResults] = useState<D1QueryResult | null>(null);
  ...
  const loadTables = async () => {
    const tableList = await listD1Tables();
    setTables(tableList);
  };
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-3xl font-bold">D1 Database</h2>
          </div>
          <button
            onClick={loadTables}
                  ...
                  <div className="space-y-2">
                    {results.meta && (
                      <div className="text-sm text-white/60 mb-4">
                        Changes: {results.meta.changes} | Duration: {results.meta.duration}ms
                      </div>
                    )}
